            ...
            <div class="col-12">
                <div class="card mb-4">
                    <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                        <h6>{{ $title }} Data</h6>
                        <a href="{{ route('couriers.create') }}" class="btn btn-primary btn-sm mb-0"> Add New {{ $title }}</a>
                    </div>
                            ...
                            <table class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-uppercase text-primary text-xs font-weight-bolder opacity-7">Vehicle Type</th>
                                        <th class="text-uppercase text-primary text-xs font-weight-bolder opacity-7">License Plate</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($datas as $nmr => $data)
                                        <tr>
                                            ...
                                            <td class="text-xs font-weight-bold mb-0">
                                                <a href="{{ route('couriers.edit', $data->id) }}" class="me-2">
                                                    <img src="{{ asset('be/assets/img/icon/edit.png') }}" alt="edit" width="20">
                                                </a>
                                                <form action="{{ route('couriers.destroy', $data->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            <td class="text-xs font-weight-bold mb-0">{{ $data->vehicle_type }}</td>
                                            <td class="text-xs font-weight-bold mb-0">{{ $data->license_plate }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
